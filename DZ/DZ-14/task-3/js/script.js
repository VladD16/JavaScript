...
// міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну (old_price),
...
if (confirm('Почати тестування?')) {
  ...
  let totalPrice = dataList.reduce(
    (prevTotalPrice, goods) => prevTotalPrice + goods.price,
    0
  )

  // 2) === Знайти кількість товарів, у яких ціна зменшилась (price < old_price ====
  let saleGoodsQuantity = dataList.reduce(
    (prevQuantity, goods) =>
      goods.price < goods.old_price ? prevQuantity + 1 : prevQuantity,
    0
  )

  // === 3) Товари, які доступні (sell_status:"available") ===
  const availableGoods = dataList.filter(
    (goods) => goods.sell_status === 'available'
  )
  ...
  // міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну (old_price),
  // та ціну у доларах (usd_price) ===
  const copyDataList = JSON.parse(JSON.stringify(dataList)).filter(
    (goods) => goods.sell_status === 'available'
  )

  const formattedDataList = []
  copyDataList.forEach((goods) => {
    let { id, price, old_price, usd_price } = goods
    goods = {
      id,
      price,
      old_price,
  ...
  document.write(`<p>
    1) ${totalPrice} <br>
    2) ${saleGoodsQuantity} <br>
    3) Рішення в консолі <br>
    4) Рішення в консолі <br>
    </p>`)
}
